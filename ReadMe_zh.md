...
另一种情况是使用高动态范围图像进行输入，比如采用 PQ 传递函数编码的 HDR 图像。首先将非线性编码的 RGB 像素值应用 EOTF 解码，得到线性 RGB 像素值。然后将线性 RGB 像素值转换到 XYZ 空间，得到三刺激值。项目中提供的几个 tif 文件和 `image_input.py` 就使用了这种输入。
...
1. 如何在显示器上再现真实世界的场景
2. 如何在低动态范围的传统显示器上再现高动态范围的图像

不同于计算机视觉，色彩科学更加关注人眼的感知，希望从视觉的角度处理图像。iCAM06 通过色适应、色调压缩、均匀颜色空间等方法，可解释的完成了从高动态范围到低动态范围的图像处理。

但是，iCAM06 也有一些不足之处：

1. 色适应的算法存在问题，且修正后的效果不理想，可能是因为色适应模型的局限性和灰度世界假设的影响。
2. 色调压缩使用的 Sigmoid 函数过多的降低了图像的对比度，且不能够兼顾低动态范围输入和高动态范围输入的效果。
3. 保边变换和细节增强可能带来伪影和过多的锐化效果。
4. 均匀颜色空间上的处理实际上没有可靠的理论基础，尤其是在明度上应用 gamma 指数的做法。

总的来说，iCAM06 借助色彩科学的研究，提出了一种有效的高动态范围图像处理方法，是将色彩科学融入图像处理的成功探索。
