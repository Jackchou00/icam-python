...
def blur(img, d):
    ...
    # Right mirror
    Y[yDim // 2 : yDim // 2 + yDim, xDim // 2 + xDim :, :] = img[
        :, xDim // 2 : xDim, :
    ][:, ::-1, :]
    # Top mirror
    Y[: yDim // 2, xDim // 2 : xDim // 2 + xDim, :] = img[: yDim // 2, :, :][::-1, :, :]
    # Bottom mirror
    Y[yDim // 2 + yDim :, xDim // 2 : xDim // 2 + xDim, :] = img[yDim // 2 :, :, :][
        ::-1, :, :
    ]
    # Top-left corner
    Y[: yDim // 2, : xDim // 2, :] = img[: yDim // 2, : xDim // 2, :][::-1, ::-1, :]
    # Top-right corner
    Y[: yDim // 2, xDim // 2 + xDim :, :] = img[: yDim // 2, xDim // 2 : xDim, :][
        ::-1, ::-1, :
    ]
    # Bottom-right corner
    Y[yDim // 2 + yDim :, xDim // 2 + xDim :, :] = img[
        yDim // 2 :, xDim // 2 : xDim, :
    ][::-1, ::-1, :]
    # Bottom-left corner
    Y[yDim // 2 + yDim :, : xDim // 2, :] = img[yDim // 2 :, : xDim // 2, :][
        ::-1, ::-1, :
    ]
    # Gaussian Filtering
    ...
    for channel in range(3):
        white[:, :, channel] = np.maximum(
            np.real(np.fft.ifft2(np.fft.fft2(Y[:, :, channel]) * filter_kernel)), 0
        )
